    ...
    var _expect = function (passed, failMsg, options)
    {
        ...
        else
        {
            ...
            if (options.extraCredit)
            {
                ...
                console.error("<MISSED::>" + Test.format(failMsg));
                incorrect++;
                globalOptions.totalIncorrect++;
            }
            else
            {
				incorrect++;
				globalOptions.totalIncorrect++;
                console.error("<FAILED::>" + Test.format(failMsg));
...
//    };
    ...
    var Test = {
    	setGlobalOptions: options => Object.assign(globalOptions, options),
        ...
        describe: function (msg, fn)
        {
            ...
            finally
            {
                ...
                if (!globalOptions.hideTiming) {
					console.log("<COMPLETEDIN::>" + ms);
				}
                if (incorrect) {
					console.log(`${msg} - # incorrect:${incorrect}`);
				}
                describing = false;
                beforeCallbacks = [];
                afterCallbacks = [];

                console.log(`correct: ${globalOptions.totalCorrect}; incorrect: ${globalOptions.totalIncorrect};`)
            }
        },
        it: function (msg, fn)
        {
...
{
    throw ["Failed to load core API methods", ex];
}
